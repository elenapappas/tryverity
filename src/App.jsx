...
function computeVerityScore(formData) {
  // ── Feature Extraction ────────────────────────────────────────────────
  // num(): respects 0 as a real value — `parseFloat(x) || d` wrongly treats 0 as missing
  const num = (v, d) => { const n = parseFloat(v); return Number.isFinite(n) ? n : d; };

  const sleep = num(formData.sleepHours, 7);
  ...
  let testCount = 0;
  const negated = /\b(no|none|nothing|zero|n\/a)\b/.test(testsText);
  const digitMatch = testsText.match(/\d+/);
  if (negated && !digitMatch) {
    testCount = 0;
  } else if (digitMatch) {
  ...
  // Projects: if the text contains a number, use it as the count ("0" → 0,
  // "3 projects" → 3) — same convention as the tests field. Otherwise filter
  // negations and count listed items.
  let projectCount = 0;
  const projDigit = projectText.match(/\d+/);
  if (projDigit) {
    projectCount = parseInt(projDigit[0]);
  } else if (projectText && !/^(no|none|nothing|nada|zero|n\/a|-)( .*)?$/i.test(projectText)) {
    projectCount = projectText.split(/,|and\s/i)
      .map(s => s.trim())
      .filter(s => s.length > 0 && !/^(no|none|nothing|nada|zero|n\/a|-)( .*)?$/i.test(s))
      .length;
  }
  projectCount = Math.min(projectCount, 6); // cap

  const assignmentCount = Math.min(num(formData.assignments, 0), 15);
  const hwNightly = Math.min(num(formData.homeworkHours, 2), 12);
  ...
  const extracurricular = num(formData.practiceHours, 0);
  const screenDaily     = num(formData.screenTime, 3);   // hours per day
  const screenWeekly    = screenDaily * 7;
  const socialStress    = num(formData.socialStress, 0); // 0–10
